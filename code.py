...
class Plot:
    ...
    def plot_user_transactions_pie_chart(data):
        ...
        Returns:
            matplotlib.figure.Figure: A pie chart showing the spending amount for each user.
        """
        df = pd.DataFrame(data, columns=['user_id', 'transaction_amount'])
        df['transaction_amount'] = pd.to_numeric(df['transaction_amount'])
        user_totals = df.groupby('user_id').sum()
        fig, ax = plt.subplots(figsize=(10, 6))
        user_totals.plot(kind='bar', y='transaction_amount', ax=ax, legend=False)
        ax.set_title('Total Spending by User')
