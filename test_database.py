...
class TestDatabase(unittest.TestCase):
    ...
    def test_fetch_current_month_transactions(self, mock_connect):
        ...
        mock_connect.assert_called_once_with(**self.mock_config)
        self.mock_cnx.cursor.assert_called_once()
        # self.mock_cursor.execute.assert_any_call("SELECT transaction_date, transaction_amount FROM transactions WHERE MONTH(transaction_date) = 1 AND YEAR(transaction_date) = 2021")
        self.assertEqual(result, [
            ("2021-01-01", 100),
            ("2021-01-02", 200),
            ("2021-01-03", 300)
